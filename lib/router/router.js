...
Router.prototype._dispatch = function(err, req, res, next){
  ...
  (function pass(i, err){
    ...
    if( req.transactionId && req.transactionId in self.mapUacTransaction ) {
    ...
    else if( -1 != ['CANCEL','PRACK','UPDATE'].indexOf(req.method) && 
      req.transactionId in self.mapUasTransaction && 
      req.method in self.mapUasTransaction[req.transactionId] ) {
      ...
      req.route = route = obj[req.method] ;
      debug('found request-level route for %s', req.method) ;
    }
    else {
      /* app level routes */
      req.route = route = self.matchRequest(req, i);
      if( req.route ) {
        debug('found app-level route for %s', req.method) ;
       }
    }

    /* clear any transaction state once invite transaction is concluded */
    if( 'ACK' === req.method && 'network' === req.source ) {
      delete self.mapUasTransaction[req.transactionId] ;
      debug('received ACK, clearing mapUasTransaction, size is now %d', Object.keys(self.mapUasTransaction).length) ;
    }
    ...
    function callbacks(err) {
      ...
      try {
        if (err && fn) {
          ...
          else if( fn === route.callbacks[numCallbacks-1] ) return fn( null, req, res, callbacks) ;
