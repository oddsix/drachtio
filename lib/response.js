...
Response.prototype.send = function( code, status, opts ) {
	...
	opts.body = Buffer.isBuffer(this.body) ? this.body.toString('utf-8') : this.body ;
...
Response.prototype.ack = function( opts, callback ) {
	...
	opts.method = 'ACK' ;
	opts.headers['call-id'] = this.get('call-id') ;

	this.agent.sendRequest( 'sendSipRequest', opts, opts.body ? function(){} : undefined ) ;

	/* if a callback is provided, just provide the final response */
